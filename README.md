...
| 课程要求                                       | 项目对应实现                                                                     |
| ---------------------------------------------- | -------------------------------------------------------------------------------- |
| 面向真实场景发现问题，明确目标和边界           | 聚焦招股说明书阅读成本高的问题，明确只做公开资料整理和初步分析                   |
| 使用智能体工具设计逻辑结构工作流               | PDF 上传、解析、清洗、LLM 抽取、报告生成、飞书推送、人工复核                     |
| 开发简单应用界面、辅助工具或数据处理脚本       | Streamlit 应用、CLI 脚本、静态展示网站                                           |
| 使用智能体辅助代码生成、解释、修复、优化或测试 | `docs/ai_development_log.md` 记录 AI 辅助开发过程                              |
| 将智能体结果与飞书协同结合                     | `agent/feishu_bot.py` 和 `scripts/test_feishu.py`                            |
| 使用 CLI 完成运行、接口测试、脚本执行          | `scripts/run_pipeline.py`、`scripts/test_api.py`、`scripts/test_feishu.py` |
| 通过测试与迭代提高稳定性                       | `docs/test_log.md` 设计异常场景和优化措施                                      |
| 形成完整文档、展示材料和反思总结               | README、docs 文档、静态网页、document.pdf                                        |
